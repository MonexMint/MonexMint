...
export function calculateCompoundInterest(
  ...
  return {
    ...
    principal,
    totalInterest: Math.round(totalInterest * 100) / 100,
    effectiveRate: Math.round(effectiveRate * 100) / 100,  // ✅ FIXED NAME
    absoluteReturn:
      Math.round((totalInterest / principal) * 100 * 100) / 100,
  };
}
...
export function calculateGoalPlanning(
  ...
  if (monthlyRate === 0) {
    monthlySIP = futureValue / totalMonths;
  } else {
    monthlySIP =
  ...
  // 🔹 3. Lumpsum Required Today
  const lumpsumRequired =
    futureValue / Math.pow(1 + annualReturnRate / 100, tenureYears);
...
// ─────────────────────────────────────────────────────────────
// 2. Salary Calculator — Used by SalaryCalculator.jsx
// ─────────────────────────────────────────────────────────────
export function calculateSalary(
  ...
  // 🔹 Basic Salary (40% of CTC typical)
  ...
  // 🔹 EPF
  const epfEmployee = pfOptOut ? 0 : basicSalary * 0.12;
  const epfEmployer = pfOptOut ? 0 : basicSalary * 0.12;

  // 🔹 Special Allowance (remaining)
  const specialAllowance =
    ctc - basicSalary - hra - gratuity - epfEmployer - bonus;
  ...
  // 🔹 Taxable Income
  let taxableIncome =
    ctc - epfEmployer - gratuity - professionalTax - epfEmployee;

  // 🔹 Standard Deduction
  taxableIncome -= 50000;

  // 🔹 Income Tax Calculation (Simplified Slabs)
  let incomeTax = 0;
  ...
  const totalDeductions =
    epfEmployee + professionalTax + incomeTax;

  const annualInHand =
    ctc - epfEmployer - gratuity - totalDeductions;

  const monthlyInHand = annualInHand / 12;

  const takeHomePercent =
    ((annualInHand / ctc) * 100).toFixed(1);
